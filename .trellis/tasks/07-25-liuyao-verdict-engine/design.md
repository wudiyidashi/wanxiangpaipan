...
| L4 变爻 | 回头生/化进神/化扶 | 回头克/化退神 | dongBian 类 |
...
- 忌神动而元神亦动 → 五行环上忌必生元（接续相生），jiActive 不构成降级，nuance="忌化为助"
  （决策表中体现为 yuanActive 行序先于 jiActive 行）
- 元神/忌神静而不空 → 不参与（静不作用）
...
首批 ≥8 例覆盖：日月生扶忌静（可成）、月破无救（难成）、旬空待出（待条件）、
忌神独发（难成）、元神动先难后成、合住待冲、伏神待出、用神化回头克。
断例按《增删卜易》原则构造并注明依据章节，**领域正确性由用户复核夹具注释**后生效。
