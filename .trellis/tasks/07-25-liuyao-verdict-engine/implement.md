...
- [x] **review gate：黄金断例领域复核完成**（2026-07-25；40 例逐例注明问事、月日、卦与动爻、取用、章节及校对本印刷页码）
  - 先修断例：元神例补旬空可解条件；月破例替换为原书“辰月戊子日占父何日归”；补大有旺空、飞伏得生、动爻日合三例
  - 再修引擎：原“化合=合绊待冲”与《六合章》不符，改为“化合=化扶”；真正合住/合绊仍待冲开
  - 扩展为覆盖矩阵：26 个原书占例 + 14 个“章法校验例”；设置 11 个规则域的最小数量断言，防止后续静默删减
  - 扩展复核先定夹具再修引擎：回头生/克前置、克处逢生、元忌非接续时忌神优先、伏神得出覆盖飞克伏无解
- 验证：`flutter test`
