...
- 当前发布常量为 `daliuren-classics/1.0.0`、`daliuren-pan/2.0.0`、snapshot `2.0.0` 和 `daliuren-analysis-project-v1/1.0.0`。C01 具名保留 `panV1=daliuren-pan/1.0.0` 与 `castInputSchemaV1=1.0.0`；发布后不原地改义。
- 盘面执行来源必须用 `DlrRuleRef.projectPan()` 生成 `dlr.project.pan.*@daliuren-pan/2.0.0`；`DlrRuleRef.project()` 的默认规则集属于 analysis，不能用于月将或历法执行身份。自动月将可把 `pan.001/.002` 记录为非执行 attribution，但它们不得成为 `executionRuleRef`。
...
- `daliuren_result_versioning_test.dart`：v2 current、C01 v1、legacy/future JSON、UTC wire、`civilTime`/typed resolution、`recastFromId`、snapshot round-trip 和外部 Map 隔离。
