...
void main() {
  ...
  group('MuJueService 入墓', () {
    test('甲戌日四爻午火：入日墓（火墓于戌）', () {
      ...
      final tags = MuJueService.analyzeYao(
          qianStatic.yaos[3], qianStatic, lunar);
      final terms = tags.map((t) => t.term);
      expect(terms, contains('入日墓'));
      expect(tags.firstWhere((t) => t.term == '入日墓').polarity,
          Polarity.xiong);
    });

    test('戌月四爻午火：入月墓', () {
      final lunar = buildLunar(yueJian: '戌', riGanZhi: '甲子');
      final terms = MuJueService.analyzeYao(
          qianStatic.yaos[3], qianStatic, lunar).map((t) => t.term);
    ...
    test('上爻戌动，四爻午火：入动墓且关联上爻', () {
      ...
      final tags = MuJueService.analyzeYao(
          qianTopMoving.yaos[3], qianTopMoving, lunar);
    ...
    test('墓支自身不入自己的动墓', () {
      final lunar = buildLunar(yueJian: '寅', riGanZhi: '甲子');
      final terms = MuJueService.analyzeYao(
          qianTopMoving.yaos[5], qianTopMoving, lunar).map((t) => t.term);
  ...
  group('MuJueService 出墓', () {
    ...
    test('未冲墓则无出墓', () {
      final lunar = buildLunar(yueJian: '寅', riGanZhi: '甲子');
      final terms = MuJueService.analyzeYao(
          qianTopMoving.yaos[3], qianTopMoving, lunar).map((t) => t.term);
