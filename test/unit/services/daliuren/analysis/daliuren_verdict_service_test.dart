...
DlrAnalysisTag tag(String term, Polarity polarity,
...
void main() {
  group('DaLiuRenVerdictService 决策表逐行命中（design §4）', () {
    ...
    test('行2：传归克身且干上克身 → 难成（内外交攻）', () {
      final j = judge(
        ganZhiTags: [tag('干上克身', Polarity.xiong)],
        juTags: [tag('传归克身', Polarity.xiong)],
      );
      expect(j.trend, VerdictTrend.nanCheng);
    ...
    test('行3：递克传退且课格凶 → 难成', () {
      final j = judge(
        keGe: keGeXiong,
        juTags: [tag('递克传退', Polarity.xiong)],
      );
      expect(j.trend, VerdictTrend.nanCheng);
    ...
    test('行4：传归生身且无悬置 → 可成；课格重审 nuance=先难后成', () {
      final j = judge(juTags: [tag('传归生身', Polarity.ji)]);
      expect(j.trend, VerdictTrend.keCheng);
      expect(j.nuance, '先难后成');
    });

    test('行4：传归生身且课格非涉害/重审 → 可成且无 nuance', () {
      final j = judge(keGe: keGeJi, juTags: [tag('传归生身', Polarity.ji)]);
      expect(j.trend, VerdictTrend.keCheng);
    ...
    test('行6：递生传进且无悬置且无克身标签 → 可成', () {
      final j = judge(juTags: [tag('递生传进', Polarity.ji)]);
      expect(j.trend, VerdictTrend.keCheng);
    ...
    test('行8：发用克身无悬置 → 难成', () {
      final j = judge(chuanTags: {
        ChuanPosition.chu: [tag('发用克身', Polarity.xiong)],
      });
      expect(j.trend, VerdictTrend.nanCheng);
    ...
    test('行9：课格吉且无任何凶标签 → 可成', () {
      final j = judge(
        keGe: keGeJi,
        ganZhiTags: [tag('干上生身', Polarity.ji, category: DlrTagCategory.ganZhi)],
      );
      expect(j.trend, VerdictTrend.keCheng);
    });
    ...
    test('行10：其余（课格中性带凶标签但不成克身格局）→ 趋势不明', () {
      final j = judge(
        ganZhiTags: [
          tag('事体受制', Polarity.xiong, category: DlrTagCategory.ganZhi),
        ],
      );
