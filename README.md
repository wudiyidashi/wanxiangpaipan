...
- 六爻分析升级为完整的古籍规则体系，以《增删卜易》为主干，并明确区分《卜筮正宗》补充规则与项目约定。
- 程序会按用神、日月、动变、伏神和有向生克形成裁决、未决条件与应期观察窗；日尺度可查看日历角标，月尺度可查看整月提示。
- AI 只解释程序给出的盘面、用神、裁决、条件、应期和来源，自定义提示词也不能要求它重算或改写这些结果。
- 术语说明新增古籍或项目来源、章节定位、证据形态与采用边界，未核验内容不会显示成逐字原文或伪造页码。
- 新建和重新生成的 AI 解读使用新版结构化提示词，既有对话继续保留原提示词与分析版本。
- 新增可选的真实模型对比评测工具，用统一古例检查事实忠实、条件覆盖、来源引用和应期边界。
