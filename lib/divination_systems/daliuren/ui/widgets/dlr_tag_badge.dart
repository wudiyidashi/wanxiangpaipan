...
/// 标签极性 → 颜色：吉=靑绿、凶=朱砂、中性=灰（对齐六爻徽标配色）
Color dlrPolarityColor(Polarity polarity) {
  switch (polarity) {
    case Polarity.ji:
...
/// 大六壬分析徽标（迷你 chip），与六爻爻行徽标同视觉
class DlrTagBadge extends StatelessWidget {
  const DlrTagBadge({
    super.key,
    required this.tag,
    this.onTap,
  ...
  /// 紧凑模式用于传行内联，false 用于详析 Sheet
  final bool dense;

  @override
  Widget build(BuildContext context) {
    final color = dlrPolarityColor(tag.polarity);
    final badge = Semantics(
      label: '${tag.term}（${tag.polarity.name}）',
      child: Container(
        padding: dense
